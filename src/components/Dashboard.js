...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [summaryRes, ieChartRes, expenseCatRes] = await Promise.all([
                    fetch(`${API_URL}/api/dashboard/summary`, { headers: { 'x-auth-token': token } }),
                    fetch(`${API_URL}/api/dashboard/charts/income-expense`, { headers: { 'x-auth-token': token } }),
                    fetch(`${API_URL}/api/dashboard/charts/expense-by-category`, { headers: { 'x-auth-token': token } })
                ]);

                if (!summaryRes.ok || !ieChartRes.ok || !expenseCatRes.ok) throw new Error('Error al cargar los datos del dashboard.');
    ...
    // 1. PRIMERA COMPROBACIÓN: Si está cargando, muestra un mensaje.
    if (loading) {
        return <p>Cargando dashboard...</p>;
    }

    // 2. SEGUNDA COMPROBACIÓN (LA CLAVE): Si, después de cargar, alguno de los datos es nulo (por un error en la API), muestra un mensaje de error y no continúa.
    if (!summaryData || !incomeExpenseData || !expenseCategoryData) {
        return <p>Error: No se pudieron cargar los datos del dashboard. Intenta recargar la página.</p>;
    }

    // 3. Si ambas comprobaciones pasan, es seguro renderizar el contenido.
    return (
        <div className="dashboard-container">
            ...
            <div className="kpi-cards-grid">
                <div className="kpi-card">
                    ...
                    <p className="value positive">${summaryData.totalRevenue.toLocaleString('es-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
                <Link to="/invoices?status=Pendiente" className="kpi-card-link">
                    <div className="kpi-card">
                        <h3>Cuentas por Cobrar</h3>
                        <p className="value warning">${summaryData.totalReceivable.toLocaleString('es-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                </Link>
